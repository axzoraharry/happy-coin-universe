...
-- Fix the card number generation function to ensure uniqueness
...
BEGIN
  LOOP
    ...
    -- Check if any card exists with this number by checking all existing cards
    -- Since card numbers are encrypted, we need to check each one
    SELECT EXISTS(
...
-- Update the issue_virtual_card function to use better salt generation
